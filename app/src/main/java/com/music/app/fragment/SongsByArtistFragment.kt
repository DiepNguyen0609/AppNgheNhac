...
import com.music.app.databinding.FragmentSongsByArtistBinding
...
class SongsByArtistFragment : Fragment() {

    private var binding: FragmentSongsByArtistBinding? = null
    private var mListSong: MutableList<Song>? = null
    private var mSongAdapter: SongAdapter? = null
    private var mArtistId: Long = 0

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentSongsByArtistBinding.inflate(inflater, container, false)
        loadDataIntent()
        initUi()
        initListener()
        loadListSongsByArtist()
        return binding?.root
    }

    private fun loadDataIntent() {
        val bundle = arguments ?: return
        mArtistId = bundle.getLong(Constant.ARTIST_ID)
    }
    ...
    private fun loadListSongsByArtist() {
    ...
    companion object {
        @JvmStatic
        fun newInstance(artistId: Long): SongsByArtistFragment {
            val fragment = SongsByArtistFragment()
            val bundle = Bundle()
            bundle.putLong(Constant.ARTIST_ID, artistId)
